...
存档走**浏览器本地 IndexedDB**，支持多存档与 JSON 导入导出。

会话、消息、变量快照、存档载荷这些「会随游戏推进持续增长」的数据都放 IndexedDB；
设置、音量、界面偏好这类小配置留在 localStorage。这么做是因为 localStorage 只有 5MB 硬上限，
聊一两回合就能触顶，而 IndexedDB 的配额按磁盘比例给。

老用户的数据在应用启动时自动迁移，不需要任何手动操作。
