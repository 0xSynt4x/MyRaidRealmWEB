...
🔴 **前两项无条件生效，不在开关里。** `$` 前缀本就不该发给 AI；生存系统关着时那几个数值
在游戏里毫无意义，发给正文只会诱导它写出「你的血量是 100」这类出戏内容。
这两项是正确性要求而非偏好，所以从可选开关里拿掉了 —— 写进开关只会让它们被误关掉。
...
🔴 **总开关默认关闭。** 它控制的是上表「受总开关控制」那一列；关掉时**只做两项无条件裁剪**，
其余保持原样（含美化输出）。子开关默认全开 —— 一旦打开总开关就是要完整效果。
