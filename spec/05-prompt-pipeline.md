...
| 项 | 正文链 | 变量更新链 | 开关 |
| --- | --- | --- | --- |
| 剔除 `$` 开头的键（酒馆助手约定：不发给 AI） | ✅ | ✅ | 无条件 |
| 生存状态按模式裁 | ✅ | ✅ | 无条件 |
| 去 JSON 缩进（紧凑输出） | ✅ | ✅ | 无条件 |
| 剔除「设置」块 | ✅ | ❌ **必须保留** | 无条件 |
| 商城只留「物品 / 技能」两个空路径 | ✅ | ✅ | 无条件 |
| NPC 按在场裁 | ❌ 全发 | ✅ 只留本轮正文提到过的 | 受唯一开关控制 |

🔴 **除最后一项外全部无条件生效，没有开关。** 它们要么是正确性要求 —— `$` 前缀本就不该发给 AI、
生存系统关着时那几个数值在游戏里毫无意义（发给正文只会诱导它写出「你的血量是 100」这类出戏内容）；
要么是纯收益无风险的整形（紧凑输出、商城只留路径）。做成开关只会让「必须做的事」变成「可能没做」。
...
⚠️ 注意：开局预设里绝大多数 NPC 都标着「重要NPC: true」，所以这条兜底在开局阶段基本等于不裁；
游戏过程中新生成的 NPC 默认 `重要NPC: false`，对新角色才真正生效。

实现：`runtime/standaloneSnapshotTrim.ts`（整形纯函数 + 按链路组装），
在 `runtime/standaloneTurn.ts` 两条链里接入。

🔴 **唯一的用户开关是「只发在场 NPC」，默认打开，且只作用于变量更新链。**
开关入口在**主界面右侧面板 → 功能设置**标签页的「上下文裁剪」卡片。
开局向导的设置页里没有这一组（那是给首次开局用的，不该塞这种进阶开关）。

🔴 **写入层护栏也是无条件的**（`runtime/standaloneTurn.ts` / `useMessageActions.ts` 里的补丁守卫）：
路径中含 `_` 开头字段段的补丁一律丢弃；生存系统关闭时指向「生存状态」的补丁一律丢弃。
