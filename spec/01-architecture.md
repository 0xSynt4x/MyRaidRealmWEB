...
src/index.ts          第 1 行 import './standalone-globals'（全局垫片，必须最先）
  ...
  ↓
initializeStandaloneStorage()   🔴 必须在挂载前 await（见 08-state-and-save.md「本地存储」）
  ↓                             把老用户的 localStorage 数据迁进 IndexedDB，
  ↓                             并把会话 / 消息 / stat_data 读进内存缓存
  ↓
...
| 状态层   | `src/stores/`                 | Pinia store：设置、消息、变量、开局、布局…    | 不直接调模型；持久化统一走 `utils/standaloneStorage.ts` |
| 运行时层 | `runtime/`                    | 组装提示词、调 Provider、跑回合、解析变量补丁 | **不依赖任何 Vue 组件**，可独立测试                     |
| 资产层   | `src/assets/`、`src/presets/` | 世界书、本地内容、预设、图标                  | 纯数据，不含逻辑                                        |
| 工具层   | `src/utils/`                  | 存储、存档、格式化、生图、提示、世界书桥接…   | 无状态优先                                              |
| 视图层   | `src/components/`             | 组件树                                        | 只读 store，不直接读写存储                              |
...
- **设置**：`stores/settings.ts`，唯一持久化点。字号、行距、主题、语言、API 配置、本地内容开关都在这里，走 localStorage。
...
- **本地存储**：`src/utils/standaloneStorage.ts` 是唯一落盘入口，`src/utils/standaloneIndexedDb.ts` 是它下面的 IndexedDB 封装。

改动状态时保持「谁写谁负责」：不要从组件里绕过 store 直接改 `stat_data` 或底层存储。

## 硬边界

1. **不引入任何外部运行时或脚本依赖。** 所有能力都在本仓内实现。
2. **不引入运行时外部依赖。** 所有依赖一律打进产物，构建配置里**没有 `externals`**（见 `02-build-and-deploy.md`）。
3. **保持单文件产物。** 任何新增依赖都要先确认它能被内联。
