...
🔴 **补丁是路径级写入，打在完整数据上。** 快照里没有的路径照样写得进去 ——
所以「发送前裁掉」不等于「写不进来」，必须**在应用补丁前再过一道过滤**。

两道拦截（各有独立开关，随发送前整形总开关一起生效）：

| 拦截 | 规则 |
| --- | --- |
| `_` 前缀路径 | 丢弃。项目原本只在提示词里约定「`_` 开头 AI 不可更新」，机制没拦，这道补上 |
| `生存状态` 路径 | 仅在生存系统**关闭**时丢弃（基础 / 生存模式正常写） |

被丢弃的操作**静默跳过、不报错**，其余操作照常应用。

实现在 `src/utils/variableUpdate.ts` 的 `filterVariableUpdatePatch()`，应用补丁的入口统一走它。
前端把补丁重放到实时状态时（`src/composables/useMessageActions.ts`）走同一套过滤，口径不分叉。
